...
class RetrievalService:
    ...
    async def retrieve(
        ...
        result.faiss_available = faiss_svc.is_available()

        if not result.faiss_available:
            logger.warning("[Retrieval] FAISS unavailable — returning empty result.")
            return result

        raw_chunks = faiss_svc.search(
