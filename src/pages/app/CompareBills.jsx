...
import { GitCompare, FileText, CheckCircle2, ShieldAlert, Loader2 } from "lucide-react";
...
export default function CompareBills() {
  ...
  useEffect(() => {
    getBills().then(({ bills }) => setBills(bills)).catch(console.error);
  }, []);
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      const result = await compareBills([selectedBill, "second_bill"]);
    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-border pb-6">
        ...
        <div className="flex gap-4">
          ...
          >
            {bills.length > 0 
              ? bills.map(b => <option key={b._id} value={b._id}>{b.title}</option>)
              : <>
          ...
          <button 
            ...
            disabled={comparing}
            className="px-5 py-2.5 rounded-lg bg-accent text-[#0a0a0f] text-sm font-bold hover:bg-accentHover shadow-glow-accent disabled:opacity-70 flex items-center gap-2"
          >
            {comparing ? <><Loader2 className="w-4 h-4 animate-spin" /> Comparing...</> : "Run AI Diff"}
          </button>
        </div>
      </div>
